...
# ============================================================
# 5) Model: EEGNet (PyTorch)
# ============================================================
class EEGNet(nn.Module):
    """
    EEGNet-ish architecture adapted for windowed raw EEG (C,T) input.

    Input to forward(): (B, C, T)
    We treat it as (B, 1, C, T) for 2D convs:
      - temporal conv across T
      - depthwise spatial conv across C
      - separable conv (depthwise temporal + pointwise)
      - global pooling -> classifier

    Lightweight by design (depthwise separable convs).

    Args:
      chans: number of EEG channels (C)
      samples: number of time samples per window (T) [optional; only used for sanity, not required]
      num_classes: 2 for AD vs CN
      F1: number of temporal filters
      D: depth multiplier (spatial filters per temporal filter)
      F2: pointwise filters after separable conv (commonly F2 = F1*D)
      kernel_length: temporal kernel length (in samples)
    ...
    def __init__(
        ...
        # Block 2: Depthwise Spatial Conv across channels
        # depthwise: groups=F1 (each temporal filter has its own spatial filters)
        # (B,F1,C,T) -> (B,F1*D,1,T)
        self.block2 = nn.Sequential(
            nn.Conv2d(
                F1, F1 * D, kernel_size=(chans, 1),
        ...
        # Block 3: Separable Conv (depthwise temporal + pointwise)
        # depthwise temporal conv over T
        # (B,F1*D,1,T/4) -> (B,F1*D,1,T/4)
        ...
        # Global pooling to avoid needing fixed T
...
def main():
    ...
    # EEGNet hyperparams (lightweight defaults)
    parser.add_argument("--eegnet_F1", type=int, default=8)
    parser.add_argument("--eegnet_D", type=int, default=2)
    parser.add_argument("--eegnet_F2", type=int, default=0, help="If 0, set F2=F1*D")
    parser.add_argument("--eegnet_kernel_length", type=int, default=64)
    parser.add_argument("--eegnet_dropout", type=float, default=0.25)
    ...
    run_name = f"eegnet_{args.tag}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # match both full ('AD','CN') and single-letter ('A','C') labels
    ...
    # EEGNet init
    F2 = None if args.eegnet_F2 == 0 else int(args.eegnet_F2)
    model = EEGNet(
        chans=in_channels,
        num_classes=2,
        F1=args.eegnet_F1,
