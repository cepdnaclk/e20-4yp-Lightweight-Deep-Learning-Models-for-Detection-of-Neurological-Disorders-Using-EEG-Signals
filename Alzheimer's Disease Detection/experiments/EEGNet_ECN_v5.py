...
class ImprovedFeatureCorrectionNetwork(nn.Module):
    """
    Improved ECN:
    - gated correction
    - embedding correction + logit correction
    - richer meta features
    - conservative residual correction
    """
    def __init__(
        self,
        n_channels: int,
    ...
    def _meta_from_logits(base_logits: torch.Tensor) -> torch.Tensor:
        ...
        if base_logits.shape[1] == 2:
            logit_gap_abs = (base_logits[:, 1:2] - base_logits[:, 0:1]).abs()
        else:
            top2_logits, _ = base_logits.topk(k=2, dim=1)
            logit_gap_abs = (top2_logits[:, :1] - top2_logits[:, 1:2]).abs()

        logit_norm = torch.norm(base_logits, dim=1, keepdim=True)

        return torch.cat(
            [max_p, margin, entropy, pos_prob, logit_gap_abs, logit_norm],
            dim=1,
        )
...
def symmetric_kl_from_logits(p_logits: torch.Tensor, q_logits: torch.Tensor) -> torch.Tensor:
    p_logprob = F.log_softmax(p_logits, dim=1)
    q_logprob = F.log_softmax(q_logits, dim=1)
    p_prob = p_logprob.exp()
    q_prob = q_logprob.exp()

    kl_pq = F.kl_div(p_logprob, q_prob, reduction="batchmean", log_target=False)
    kl_qp = F.kl_div(q_logprob, p_prob, reduction="batchmean", log_target=False)
...
def evaluate_ecn(
    ...
    ecn: ImprovedFeatureCorrectionNetwork,
    loader: DataLoader,
    device: torch.device,
    criterion: nn.Module,
    use_amp: bool,
    ...
    for xb, yb, metas in loader:
        ...
        with amp_autocast(device, use_amp):
            ...
            corr_emb, delta_emb, delta_logits, gate = ecn(xb, base_emb, base_logits)
            head_logits = base_model.classify_from_embedding(corr_emb)
            corr_logits = head_logits + gate * delta_logits

            cls_loss = criterion(corr_logits, yb)
            delta_reg = reg_lambda * (delta_emb ** 2).mean()
            gate_reg = gate_lambda * (gate ** 2).mean()
            consistency_loss = consistency_lambda * symmetric_kl_from_logits(corr_logits, base_logits)

            loss = cls_loss + delta_reg + gate_reg + consistency_loss
...
def train_ecn(
    ...
    optimizer = optim.AdamW(ecn.parameters(), lr=lr, weight_decay=1e-3)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode="max", factor=0.5, patience=2, min_lr=1e-6
    )
    scaler = torch.cuda.amp.GradScaler(enabled=(use_amp and device.type == "cuda"))
    ...
    for epoch in range(1, epochs + 1):
        ...
        for xb, yb, _ in train_loader:
            ...
            delta_mag += float((gate * delta_emb).detach().abs().mean().item())
            gate_mean += float(gate.detach().mean().item())

        train_loss = run_loss / max(total, 1)
        train_acc = run_correct / max(total, 1)
        delta_mag = delta_mag / max(1, len(train_loader))
        gate_mean = gate_mean / max(1, len(train_loader))
        ...
        logger.info(
            ...
            f"train_loss={train_loss:.4f} train_acc={train_acc:.4f} "
            f"delta_mag={delta_mag:.4f} gate_mean={gate_mean:.4f} | "
            f"val_win_loss={ev['window_loss']:.4f} val_win_balacc={ev['window_bal_acc']:.4f} "
            f"val_subj_balacc={ev['subject_metrics']['balanced_acc']:.4f} "
            f"val_subj_macroF1={ev['subject_metrics']['macro_f1']:.4f} | lr={lr_now:.2e}"
        )
...
def main():
    ...
    parser.add_argument("--shards_dir", type=str, default="data/ds004504_preprocessed_shards")
    ...
    parser.add_argument("--ecn_reg_lambda", type=float, default=0.05)
    parser.add_argument("--ecn_scale_init", type=float, default=0.10)
    parser.add_argument("--ecn_consistency_lambda", type=float, default=0.02)
    parser.add_argument("--ecn_gate_lambda", type=float, default=0.005)
    ...
    for fold_id, (tr_sub_idx, va_sub_idx) in enumerate(skf.split(subjects, subj_labels), start=1):
        ...
        ecn_model = ImprovedFeatureCorrectionNetwork(
            n_channels=in_channels,
            emb_dim=128,
            n_classes=2,
        ...
        ecn_model, best_ecn_subj_balacc = train_ecn(
            ...
            consistency_lambda=float(args.ecn_consistency_lambda),
            gate_lambda=float(args.ecn_gate_lambda),
        )
        ...
        torch.save(
            {
                ...
                "model_name": "ImprovedEEGNet+ImprovedFeatureCorrectionNetwork",
                "fold": fold_id,
