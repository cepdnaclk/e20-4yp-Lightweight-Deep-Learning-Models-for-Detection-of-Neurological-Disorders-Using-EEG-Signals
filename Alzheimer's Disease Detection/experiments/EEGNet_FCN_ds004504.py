...
Three-phase training strategy:
  ...
  Phase 3 — (optional) Unfreeze all, end-to-end fine-tune at small lr

Outputs:
  logs    -> <fyp_root>/experiments/logs/train_eegnetfcn_ds004504.log
  model   -> <fyp_root>/models/train_eegnetfcn_ds004504.pt
  metrics -> <fyp_root>/experiments/train_eegnetfcn_ds004504/metrics.json
"""
...
class FeatureCorrectionNetwork(nn.Module):
    def __init__(self, n_channels: int, emb_dim: int, n_classes: int = 2, scale_init: float = 0.10):
        super().__init__()
        self.n_classes = n_classes
        self.emb_dim   = emb_dim
...
class EEGNetFCN(nn.Module):
    ...
    def __init__(
        ...
        self.fcn  = FeatureCorrectionNetwork(n_channels=n_channels, emb_dim=emb_dim, n_classes=n_classes, scale_init=scale_init)
...
# ============================================================
# Phase 3 — End-to-end fine-tune (optional)
# ============================================================
...
def main():
    ...
    # Phase 2 — FCN training
    parser.add_argument("--fcn_epochs",              type=int,   default=20)
    parser.add_argument("--fcn_lr",                  type=float, default=5e-4)
    parser.add_argument("--fcn_patience",            type=int,   default=8)
    ...
    run_name = "train_eegnetfcn_ds004504"
    ...
    logger.info(
        f"Final | val_loss={final_ev['loss']:.4f} val_acc={final_ev['acc']:.4f} "
    ...
    logger.info(f"Saved CM      : {cm_path}")
    logger.info("========== RUN END ==========")
