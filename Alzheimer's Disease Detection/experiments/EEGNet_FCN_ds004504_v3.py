#!/usr/bin/env python3
"""
EEGNet_FCN_ds004504_v3.py

Train EEGNetFCN (EEGNet base + FeatureCorrectionNetwork) on:
  <fyp_root>/data/ds004504_preprocessed_shards/{train,eval}

Improvements over v2:
  1. EEG-specific data augmentation (noise, channel dropout, time shift, amplitude scale)
  2. Label smoothing in CrossEntropyLoss to reduce overconfidence
  3. Subject-level prediction aggregation in final evaluation
  4. Relaxed FCN regularisation defaults (scale_init=0.30, lower lambdas)
  5. Phase 3 enabled by default (--skip_phase3 to disable)
  6. Larger patience / epoch budgets by default

Three-phase training strategy:
  Phase 1 — Train EEGNet base alone until convergence
  Phase 2 — Freeze base, train FeatureCorrectionNetwork only
  Phase 3 — Unfreeze all, end-to-end fine-tune at small lr

Outputs:
  logs    -> <fyp_root>/experiments/logs/EEGNet_FCN_ds004504_v3.log
  model   -> <fyp_root>/models/EEGNet_FCN_ds004504_v3.pt
  metrics -> <fyp_root>/experiments/EEGNet_FCN_ds004504_v3/metrics.json
"""
...
def augment_eeg(x: np.ndarray, fs: int = 250) -> np.ndarray:
    ...
    Augmentations applied independently with their own probabilities:
    ...
    # 3. Time shift  (up to ±200 ms = ±50 samples at 250 Hz)
    if np.random.rand() < 0.5:
        max_shift = int(0.2 * fs)   # 200 ms
...
class ShardedNpyDataset(Dataset):
    def __init__(
        ...
        self.logger     = logger
        self.training   = training          # NEW: enables augmentation
        self.fs         = fs
...
def subject_level_metrics(
    ...
    """
    Aggregate per-window softmax probabilities by subject (mean pooling),
    then predict the class with the highest mean probability.

    Returns a dict with subject-level acc, bal_acc, macro_f1,
    plus arrays y_true_subj and y_pred_subj.
    """
...
# ============================================================
# Model — Stage 2: FeatureCorrectionNetwork
# ============================================================

class FeatureCorrectionNetwork(nn.Module):
    def __init__(self, n_channels: int, emb_dim: int, n_classes: int = 2, scale_init: float = 0.30):
        super().__init__()
        self.n_classes = n_classes
        self.emb_dim   = emb_dim
...
# ============================================================
# Model — Hybrid: EEGNetFCN
# ============================================================

class EEGNetFCN(nn.Module):
    """
    Hybrid model: EEGNet base + FeatureCorrectionNetwork.
...
def hybrid_loss(
    ...
    gate_lambda: float        = 0.001,
) -> Tuple[torch.Tensor, Dict[str, float]]:
    """
    cls_loss         : CE (with label smoothing) on final_logits
    delta_reg        : L2 on delta_emb          — keeps corrections small
    gate_reg         : L2 on gate               — discourages over-correction
    consistency_loss : Symmetric KL(final||base) — prevents drift from base
    """
...
# ============================================================
# Evaluation  (window-level + subject-level)
# ============================================================
...
def train_phase1(
    ...
    logger.info("===== PHASE 1: Training base EEGNet =====")
...
def train_phase2(
    ...
    logger.info("===== PHASE 2: Training FeatureCorrectionNetwork (base frozen) =====")
...
def main():
    ...
    # Phase 2 — FCN training  (relaxed defaults vs v2)
    ...
    parser.add_argument("--fcn_reg_lambda",          type=float, default=0.02)
    parser.add_argument("--fcn_consistency_lambda",  type=float, default=0.01)
    parser.add_argument("--fcn_gate_lambda",         type=float, default=0.001)
    parser.add_argument("--fcn_scale_init",          type=float, default=0.30)

    # Phase 3 — end-to-end fine-tune  (ON by default in v3)
    parser.add_argument("--skip_phase3",       action="store_true",
                        help="Skip Phase 3 end-to-end fine-tuning")
    parser.add_argument("--finetune_epochs",   type=int,   default=15)
    parser.add_argument("--finetune_lr",       type=float, default=5e-6)
    parser.add_argument("--finetune_patience", type=int,   default=7)

    args = parser.parse_args()

    run_name = "EEGNet_FCN_ds004504_v3"
    ...
    train_ds = ShardedNpyDataset(
        ...
        training=(not args.no_augment), fs=args.target_fs,
    )
    ...
    best_p1 = train_phase1(model, train_loader, eval_loader, device, criterion, args, logger, use_amp)
    best_p2 = train_phase2(model, train_loader, eval_loader, device, criterion, args, logger, use_amp)

    best_p3 = None
    if not args.skip_phase3:
        best_p3 = train_phase3(model, train_loader, eval_loader, device, criterion, args, logger, use_amp)

    train_time = time.time() - t0
    logger.info(f"Total training time: {train_time:.2f}s")

    # ── Final evaluation (window-level + subject-level) ───────
