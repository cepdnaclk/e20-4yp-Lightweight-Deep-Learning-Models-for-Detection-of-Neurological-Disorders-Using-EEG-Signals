...
- Train Hybrid EEG Temporal Backbone + TEECNet (PyTorch + PyG) + evaluate

NO EARLY STOPPING: trains exactly --epochs epochs.
...
def build_windowed_shards(
    ...
    for set_path in subject_set_files:
        ...
        if T_total < win_len:
            logger.warning(
                f"[{split}] Too short for one window: {set_path.name} | samples={T_total} < win_len={win_len}"
            )
            too_short += 1
            continue
    ...
    if not x_files:
        raise RuntimeError(
            f"[{split}] No shards written. read_fail={read_fail} no_eeg={no_eeg} "
            f"too_short={too_short} wrote_windows={wrote_windows}"
        )
...
class TEECNetModule(nn.Module):
    ...
    def _compute_edge_attr_vectorized(self, h_bc_h: torch.Tensor) -> torch.Tensor:
        ...
        dist = torch.norm(dst_feat - src_feat, dim=-1)
        dist = dist / (dist.mean(dim=1, keepdim=True) + 1e-6)

        edge_attr = torch.stack([cos_sim, dist], dim=-1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        edge_attr = self._compute_edge_attr_vectorized(h)  # (B*E,2)
...
class TemporalDWBackbone(nn.Module):
    def __init__(self, in_chans: int, feat_dim: int = 32, kernel: int = 64, dropout: float = 0.2):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        B = x.shape[0]
        return x.view(B, self.in_chans, self.feat_dim)  # (B,C,F)
...
class HybridEEG_TEECNet(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.backbone = TemporalDWBackbone(in_chans=in_chans, feat_dim=feat_dim, kernel=backbone_kernel, dropout=backbone_dropout)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        node_feat = self.backbone(x)      # (B,C,F)
        node_feat = self.teec(node_feat)  # (B,C,F)
...
def main():
    ...
    logger.info("========== TRAINING (NO EARLY STOPPING) ==========")
    best_val_balacc = -1.0
    best_state = None
