...
v4 improvements over v3:
  - Lower initial LR (3e-4) + CosineAnnealingLR schedule
  - Gradient clipping (max_norm=1.0)
  - AdamW optimizer with weight decay (1e-2)
  - More epochs (80) and patience (12)
  - Bandpass filtering enabled by default (0.5–45 Hz)
  - Data augmentation (Gaussian noise, amplitude scaling, channel dropout)
  - Spatial filter layer in EEGNet frontend
  - LayerNorm before classifier head
  - Increased graph topk (8)
  - Differentiated dropout (frontend=0.2, GNN=0.3, classifier=0.4, TEECN=0.1)
...
def build_windowed_shards(
    ...
    for set_path in subject_set_files:
        ...
        # Bandpass filter (enabled by default in v4)
        if apply_bandpass:
...
# ============================================================
# 4) Sharded Dataset with Data Augmentation
# ============================================================
...
# ============================================================
# 5) Model: EEGNet -> GNN -> TEECN (PyTorch + PyG) — v4
# ============================================================
...
class EEGNetTemporalFrontend(nn.Module):
    """
    EEGNet-inspired temporal frontend with spatial filter (v4).
    Outputs per-channel node embeddings.
    ...
    def __init__(
        ...
        # Spatial depthwise filter (new in v4)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        y = self.spatial(y)                # (B, C*D, T)
        y = self.bn_spatial(y)
        y = F.elu(y)
        y = self.spatial_pool(y)           # (B, C*D, T')
        y = self.dropout(y)

        # Block 2: Separable conv
        y = self.separable_depthwise(y)    # (B, C*D, T')
        y = self.separable_pointwise(y)    # (B, C, T')
        y = self.bn2(y)
        y = F.elu(y)
        y = self.pool2(y)                  # (B, C, T'')
        y = self.dropout(y)

        # Pointwise embedding per channel
        T_out = y.shape[-1]
        y = y.reshape(B * C, 1, T_out)    # (B*C, 1, T'')
...
class EEGNetGNNTEECNConfig:
    ...
    temporal_kernel: int = 64
    temporal_pool: int = 4
    topk: int = 8           # v4: increased from 6
    gat_hidden: int = 64
    gat_heads: int = 4
    gnn_layers: int = 2
    frontend_dropout: float = 0.2    # v4: differentiated dropout
    gnn_dropout: float = 0.3
    classifier_dropout: float = 0.4
    teecn_dropout: float = 0.1
...
class EEGNetGNNTEECN(nn.Module):
    def __init__(self, cfg: EEGNetGNNTEECNConfig):
        ...
        # v4: LayerNorm before classifier + differentiated dropout
...
def main():
    ...
    parser.add_argument("--overlap", type=float, default=0.75)  # v4: increased from 0.5
    ...
    # Bandpass (ON by default in v4)
    parser.add_argument("--apply_bandpass", action="store_true", default=True)
    parser.add_argument("--no_bandpass", action="store_true", help="Disable bandpass filtering")
    ...
    parser.add_argument("--epochs", type=int, default=80)          # v4: 80 from 30
    parser.add_argument("--lr", type=float, default=3e-4)          # v4: 3e-4 from 1e-3
    parser.add_argument("--weight_decay", type=float, default=1e-2)  # v4: new
    parser.add_argument("--max_grad_norm", type=float, default=1.0)  # v4: new
    parser.add_argument("--patience", type=int, default=12)         # v4: 12 from 6
    ...
    # EEGNet-GNN-TEECN hyperparams — v4 defaults
    ...
    # v4: Fixed log file name
    run_name = "EEGNet_Teecnet_4504_v4"
    logger = setup_logger(fyp_root / "experiments" / "logs", run_name)

    logger.info("========== RUN START (v4) ==========")
    ...
    logger.info(f"  Bandpass       : {args.apply_bandpass} ({args.bandpass_low}-{args.bandpass_high} Hz)")
    logger.info(f"  Augmentation   : {args.augment} (noise_std={args.noise_std}, scale=[{args.scale_lo},{args.scale_hi}], ch_drop={args.channel_drop_prob})")
    ...
    logger.info("--- End v4 Improvements ---")
    ...
    eval_ds = ShardedNpyDataset(eval_dir, cache_size=args.cache_size, logger=logger, augment=False)
    ...
    # EEGNet-GNN-TEECN init — v4 config
    ...
    # Log per-module parameter breakdown
    ...
    # v4: AdamW + CosineAnnealingLR + gradient clipping
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # v4: CosineAnnealingLR steps per epoch (not on val_loss)
        scheduler.step()
