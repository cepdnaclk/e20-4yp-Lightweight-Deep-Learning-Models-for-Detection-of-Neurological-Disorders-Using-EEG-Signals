...
def build_windowed_shards(
    ...
    for set_path in subject_set_files:
        ...
        # Binary mapping (handles both full labels like 'AD'/'CN' and
        # single-letter labels like 'A'/'C' used in ds004504)
        if grp_u in ("AD", "A"):
            y_label = 1
        elif grp_u in ("CN", "C"):
            y_label = 0
        ...
        # Debug duration (super useful)
        dur_sec_orig = raw.n_times / float(raw.info["sfreq"])
        logger.info(f"[{split}] Loaded {set_path.name} | sid={sid} grp={grp_u} "
        ...
        # ✅ CRITICAL FIX: must be long enough for 1 window
        if T_total < win_len:
        ...
        for w in range(int(n_windows)):
            start = w * step_len
            end = start + win_len  # guaranteed <= T_total
...
def main():
    ...
    # ✅ FIX: match both full ('AD','CN') and single-letter ('A','C') labels
