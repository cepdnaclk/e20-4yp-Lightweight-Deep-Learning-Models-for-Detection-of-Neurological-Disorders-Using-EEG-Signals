...
============================================================
Your previous hybrid had a critical limitation:
  - It extracted a single window-level embedding (B,F),
    then *replicated it to all channels* to form (B,C,F).
  - That means all nodes had identical features, so TEECNet
    could not learn meaningful inter-channel interactions.

This script fixes that and improves TEECNet stability:

[1] REAL per-channel node features (major fix ✅)
    - Added a shared per-channel CNN encoder:
        (B,C,T) -> reshape -> (B*C,1,T) -> CNN -> (B*C,F) -> reshape -> (B,C,F)
    - TEECNet now receives true channel-specific embeddings.

[2] Sparse graph instead of fully-connected (better generalization ✅)
    - TEECNet graph is now k-NN on channel index (simple, no montage coords needed).
    - Default k=6 (configurable via --teec_k).
    - This reduces noisy edges and overfitting risk.

[3] Stable residual correction with learnable gate (prevents over-correction ✅)
    - TEECNet output uses gated residual:
        x + tanh(alpha) * delta
      instead of x + delta.
    - alpha is learnable (initialized small).

[4] LayerNorm + Dropout inside TEECNet (training stability ✅)
    - LayerNorm after input projection and each graph layer.
    - Dropout in graph block.

[5] Edge attributes improved
    - edge_attr includes:
        [cos_sim, feat_dist_norm, index_dist_norm]
      (edge_dim=3)
============================================================
"""
...
class LightweightGraphConv(pyg_nn.MessagePassing):
    ...
    """
    def __init__(self, in_channels: int, out_channels: int, edge_dim: int = 3):
...
def build_knn_edge_index(num_channels: int, k: int) -> torch.Tensor:
    """
    Simple k-NN on channel index (no montage coords required).
    For each node i, connect to k nearest indices on both sides.

    Produces directed edges (i -> j). Self-edges excluded.
    """
    k = max(1, min(k, num_channels - 1))
    edges = []
    for i in range(num_channels):
        # nearest by index distance
        candidates = [(abs(i - j), j) for j in range(num_channels) if j != i]
        candidates.sort(key=lambda x: x[0])
        nbrs = [j for _, j in candidates[:k]]
        for j in nbrs:
            edges.append([i, j])
...
class TEECNetModule(nn.Module):
    ...
    Improvements:
      - Sparse graph (kNN edges)
      - edge_attr includes [cos_sim, feat_dist_norm, index_dist_norm] => edge_dim=3
      - LayerNorm + Dropout in graph block
      - Gated residual correction (learnable alpha)
    """
    def __init__(
        self,
        num_channels: int,
        ...
        self.gn_lns = nn.ModuleList([nn.LayerNorm(hidden_dim) for _ in range(num_layers)])
        self.gn_drop = nn.Dropout(dropout)

        self.output_proj = nn.Linear(hidden_dim, feature_dim)

        # gated residual scaling (small init prevents over-correction early)
        self.alpha = nn.Parameter(torch.tensor(0.05, dtype=torch.float32))

        # sparse graph
        edge_index = build_knn_edge_index(num_channels, k)
        self.register_buffer("edge_index", edge_index)

        # precompute index distances for base graph edges (E,1)
        src, dst = edge_index[0], edge_index[1]
        idx_dist = (src - dst).abs().float().unsqueeze(1)  # (E,1)
        idx_dist = idx_dist / (idx_dist.mean() + 1e-6)
        self.register_buffer("edge_index_dist", idx_dist)  # (E,1)

    def _compute_edge_attr(self, h: torch.Tensor) -> torch.Tensor:
        """
        h: (B*C, H) after input projection
        returns edge_attr: (B*E, 3)
        """
        B = h.shape[0] // self.num_channels
        h = h.reshape(B, self.num_channels, -1)  # (B,C,H)
        ...
        for b in range(B):
            ...
            feat_dist = torch.norm(dst_feat - src_feat, dim=1, keepdim=True)       # (E,1)
            feat_dist = feat_dist / (feat_dist.mean() + 1e-6)

            # index distance from buffer (E,1)
            idx_dist = self.edge_index_dist  # (E,1)

            edge_attrs.append(torch.cat([cos_sim, feat_dist, idx_dist], dim=1))    # (E,3)

        return torch.cat(edge_attrs, dim=0)  # (B*E, 3)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        h = self.input_proj(x_flat)                     # (B*C, H)
        h = self.in_ln(h)
        h = F.relu(h)

        edge_attr = self._compute_edge_attr(h)          # (B*E, 3)
        ...
        for conv, ln in zip(self.graph_convs, self.gn_lns):
            h = conv(h, edge_index_batch, edge_attr)    # (B*C, H)
            h = ln(h)
            h = F.relu(h)
            h = self.gn_drop(h)

        delta = self.output_proj(h).reshape(B, self.num_channels, -1)  # (B,C,F)

        # gated residual: x + tanh(alpha)*delta
        gate = torch.tanh(self.alpha)
        return x + gate * delta


# ============================================================
# 5) 1D CNN + TEECNet Hybrid Model (FIXED NODE FEATURES)
# ============================================================
class CNN1D_TEECNet(nn.Module):
    ...
    FIXED approach:
      - Extract per-channel embeddings with a shared CNN:
          (B,C,T) -> (B*C,1,T) -> CNN -> pool -> (B*C,F) -> reshape -> (B,C,F)
      - Apply TEECNet on channel graph
    ...
    def __init__(
        ...
        # TEECNet over channels (now meaningful)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # TEECNet correction over channel graph
...
def main():
    ...
    parser.add_argument("--teec_k", type=int, default=6)            # NEW
    parser.add_argument("--dropout", type=float, default=0.3)
    parser.add_argument("--teec_dropout", type=float, default=0.1)  # NEW
    ...
    run_name = f"cnn1d_teecnet_v2_{args.tag}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    logger.info(
        f"Model: CNN1D_TEECNet(v2) | params={n_params:,} | "
        f"feature_dim={args.feature_dim} teec_k={args.teec_k}"
    )
