...
- Train MobileNet + TEECNet hybrid (PyTorch) + evaluate

Dependencies:
  pip install numpy pandas torch scikit-learn mne
  pip install torch-geometric  (and its required wheels for your CUDA/PyTorch)
"""
...
# ============================================================
# 5) MobileNet + TEECNet Hybrid Model (your architecture)
# ============================================================
...
class TEECNetModule(nn.Module):
    ...
    def forward(self, x):
        ...
        x_flat = x.reshape(batch_size * self.num_channels, -1)  # (B*C, F)
        ...
        h = self.output_proj(h)  # (B*C, F)
        h = h.reshape(batch_size, self.num_channels, -1)  # (B, C, F)
        return x + h


class MobileNet_TEECNet(nn.Module):
    """
    Accepts EEG windows as:
      - (B, C, T)  [from your dataloader], or
      - (B, T, C)
    Internally it uses the MobileNet stem/features then TEECNet.
    """
    def __init__(self, num_channels: int, num_classes: int = 2):
        ...
        self.stem = nn.Sequential(
            nn.Conv1d(num_channels, 32, 3, padding=1, bias=False),
            nn.BatchNorm1d(32),
        ...
        self.features = nn.Sequential(
            ...
            DepthwiseSeparableConv1D(64, 64, stride=2),
            DepthwiseSeparableConv1D(64, 128),
            DepthwiseSeparableConv1D(128, 128, stride=2),
        )

        self.channel_pool = nn.AdaptiveAvgPool1d(1)

        # TEEC over channels; feature_dim matches the MobileNet feature dimension (128)
        self.teecnet = TEECNetModule(num_channels=num_channels, feature_dim=128)
    ...
    def forward(self, x):
        # Normalize input layout to (B, T, C) then to conv layout (B, C, T)
        if x.ndim != 3:
            raise ValueError(f"Expected 3D tensor, got {tuple(x.shape)}")

        # If input is (B,C,T) (typical), convert to (B,T,C)
        ...
        x = x.permute(0, 2, 1)  # (B,C,T) for conv

        x = self.stem(x)
        x = self.features(x)                 # (B,128,T')
        x = self.channel_pool(x).squeeze(-1) # (B,128)

        # replicate feature vector across channels for graph module: (B, C, 128)
        x = x.unsqueeze(1).expand(-1, self.num_channels, -1).contiguous()

        x = self.teecnet(x)                  # (B, C, 128)

        x = x.permute(0, 2, 1)              # (B,128,C)
        x = self.post_graph(x)              # (B,256,C)
...
def main():
    ...
    # IMPORTANT: point this to the existing sharded folder on the department server
    ...
    # By default we DO NOT rebuild because your shards already exist on the server
    ...
    run_name = f"mobilenet_teecnet_{args.tag}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # Decide whether shards must be built
    ...
    x0, y0 = train_ds[0]
    in_channels = int(x0.shape[0])  # EEG channels (C)
    win_len = int(x0.shape[1])      # samples per window (T)
    ...
    # ✅ Your requested swap: CNN -> MobileNet + TEECNet
    model = MobileNet_TEECNet(num_channels=in_channels, num_classes=2).to(device)

    # Optional: log parameter count
    n_params = sum(p.numel() for p in model.parameters())
    logger.info(f"Model: MobileNet_TEECNet | params={n_params:,}")
